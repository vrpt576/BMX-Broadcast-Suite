"""Round-aware current-results API and OBS overlay."""
...
const labels={round_1:'ROUND 1',round_2:'ROUND 2',round_3:'ROUND 3',quarterfinal:'QUARTERFINALS',semifinal:'SEMIFINALS',main:'MAIN',overall:'OVERALL'};
...
function render(s,c){document.body.style.visibility=(preview||c.active_graphic==='results')?'visible':'hidden';phase.textContent=(s.phase_label||labels[s.race_phase]||s.race_phase).toUpperCase();document.querySelector('#class').textContent=s.class_name;moto.textContent=`MOTO ${s.moto_number}`;stale.style.display=s.is_stale?'block':'none';rows.replaceChildren();for(const r of s.riders){const d=document.createElement('div');d.className='row';d.innerHTML=`<div class="place">${r.finish??(r.transferred?'X':(r.status||'—'))}</div><div class="plate">${r.bike_number??'—'}</div><div class="name"></div>`;d.lastElementChild.textContent=`${r.first_name} ${r.last_name}`;rows.append(d)}}
