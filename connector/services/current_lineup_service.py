...
class CurrentLineupService:
    ...
    def __init__(
        ...
        sqorz_class_aliases: SqorzClassAliasStore | None = None,
    ) -> None:
        ...
        self.sqorz = sqorz
        self.sqorz_class_aliases = sqorz_class_aliases
    ...
    def _augment_with_sqorz(self, lineup: CurrentLineup) -> CurrentLineup:
        ...
        """
        if lineup.source == "demo":
            # Structural boundary, not a hope that matching rejects it: the
            # demo lineup's fictional riders must never be able to receive a
            # real Sqorz time, no matter what a plate/name happens to
            # coincidentally match. From the overlay there is no visible
            # difference between a real match and a coincidence, so this is
            # enforced here rather than left to the confidence tiers.
            return lineup
        ...
        try:
            ...
            if not fetch.riders:
                return lineup
            class_alias = (
                self.sqorz_class_aliases.get_alias(lineup.class_name)
                if self.sqorz_class_aliases is not None
                else None
            )
            ...
            # most recent result without recomputing it -- see
            # connector/routes/sqorz_match_report.py.
            self.sqorz.last_match_report = report
            self.sqorz.last_match_class_name = lineup.class_name
            self.sqorz.last_match_class_alias = class_alias
        ...
        return lineup.model_copy(
            update={"riders": updated_riders, "sqorz_phase_code": phase_code}
        )
